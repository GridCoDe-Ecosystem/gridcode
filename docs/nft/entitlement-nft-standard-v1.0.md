...
5. Hash-verifiable
6. Compatible with GRIDNFT Standards v1.3 — see `/docs/nft/gridnft-standards-v1.3.md`

### 1.3 Relationship to Store Protocol

Entitlement NFTs are post-settlement artefacts. They depend on the StoreVault FSM reaching `settlementFinalized = true`. They are NOT part of the core FSM.

See `/docs/store/store-v1-protocol-spec-v1.0.md` for the FSM definition.
...
The FULFILL action MUST include `deliveryProofHash`. See `/docs/store/store-v1-protocol-spec-v1.0.md`.
...
During a dispute, evidence MAY reference `deliveryProofHash`. Proof validation SHALL remain deterministic. NFT minting SHALL NOT occur until the dispute is resolved.
...
Reputation remains identity-bound. Entitlement does not equal trust score.
...
- Conform to GRIDNFT structural standards v1.3 — see `/docs/nft/gridnft-standards-v1.3.md`
...
**Document Tier:** Tier-1 — NFT & Metadata Layer
