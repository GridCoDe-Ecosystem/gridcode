...
> `AUTO_CANCEL_TIMEOUT` is not implemented in v1. Timeout-based cancellation requires a runtime scheduler that is outside the scope of a synchronous Vault FSM. If a PURCHASED listing exceeds `fulfillmentWindowMs` without a FULFILL action, the Adapter layer is responsible for flagging liveness violation. Formal auto-cancellation is deferred to v2.
...
`fulfillmentWindowMs` is enforced by the Adapter layer for liveness monitoring. It does not trigger an in-engine FSM transition in v1.
...
- Trigger AUTO_CANCEL_TIMEOUT (not implemented in v1 — deferred to v2)
...
`role` is NOT an input parameter. The engine derives role internally from `actorId` matched against listing context. `role` is not trusted as a caller-supplied value.
...
```
Valid(S, A, actorId, T, P) = true
```

Role is derived internally from `actorId`. It is not passed as an argument to the validity predicate.
...
Terminal means terminal. Once `settlementFinalized = true` is set — whether by `RESOLVE_SELLER`, `RESOLVE_BUYER`, or `FINALIZE_SETTLEMENT` — no further actions are valid on that listing, including `FINALIZE_SETTLEMENT` itself.
...
| Malicious Seller | Never fulfills | Liveness violation flagged by Adapter; buyer may dispute |
